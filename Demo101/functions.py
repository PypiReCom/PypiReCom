...
def get_llm_suggestions(Search_Context):
    prompt = f"Please provide comma-separated list of Python packages related to {Search_Context}. Output format: names of packages seperated by comma like package 1, package 2, package 3, etc."
...
def save_data(Search_Context,data,pypi_packages,llm_packages):
    '''
    Input: Space seperaetd keywords to be searched -> Search_Context (In address or any operation _ is used to join the Search_Context), 
    ...
    try:
        ...
        if name in pypi_packages:
            source += 'Pypi'
        if name in llm_packages and source == '':
            source += 'LLM'
        elif name in llm_packages:
            source += ' & LLM'
        with open(base_directory+"/Package_Basic_Data.csv","a", newline='') as file:
            csv_file = csv.writer(file)
            csv_file.writerow([name,author,author_email,license,development_status,source])
...
def generate_graph_wCustomSchema(Search_Context,schema_object):
    base_directory = parent_dir+'_'.join(Search_Context.split())
    try:
        # Extracting data from Package_Basic_Data csv
        basic_df = csv_to_df(base_directory+"/Package_Basic_Data.csv")
        dependency_df = csv_to_df(base_directory+"/Package_Dependency.csv")
        prog_lang_df = csv_to_df(base_directory+"/Package_Prog_Lang.csv")
        G = nx.DiGraph()
        nodes = schema_object["nodes"]
        edges = schema_object["edges"]
        for index in basic_df.index:
            for edge in edges:
                try:
                    # print(edges[edge][0],edges[edge][1])
                    if(edges[edge][0]!="dependency_pkg" and edges[edge][1]!="dependency_pkg" and edges[edge][0]!="language" and edges[edge][1]!="language"):
                        G.add_edge(basic_df[edges[edge][0]][index],basic_df[edges[edge][1]][index],label=edge)
                    elif(edges[edge][0]=="dependency_pkg" and edges[edge][1]=="dependency_pkg"):
                        G.add_edge(dependency_df[edges[edge][0]][index],dependency_df[edges[edge][1]][index],label=edge)
                    elif(edges[edge][0]=="language" and edges[edge][1]=="language"):
                        G.add_edge(prog_lang_df[edges[edge][0]][index],prog_lang_df[edges[edge][1]][index],label=edge)
                except Exception as e:
                    print("Exception" + e)
            ...
            for node in nodes:
                # print(df[node][index])
                attributes = {}
                for attribute in nodes[node]["attributes"]:
                    attributes[attribute] = basic_df[attribute][index]
                nx.set_node_attributes(G,{basic_df[node][index] : attributes})
        ...
        print(e)
        logging.error(f'generate_graph_wCustomSchema function - Exception: {e}')
...
def gml_to_json(Search_Context):
    ...
    try:
        ...
        with open(base_directory+"/graph.json", "w") as graphfile:
            json.dump({'result':results,'Package_Dependency':package_dependency,'license':license,'Package_Language':package_language}, graphfile)
...
def fetch_and_update_graph(Search_Context,credentials):
    ...
    if create_directory(Search_Context)['Status Code'] == 200:
        ...
        llm_packages = get_llm_suggestions(Search_Context)
        packages = [*set(pypi_packages+llm_packages)]
        package_count = 0
        ...
        for package in packages:
            ...
            try:
                ...
                save_data(Search_Context,data,pypi_packages,llm_packages)
        ...
        elif credentials["graph_db"] == "Custom":
            with open('schema_custom_config.json','r') as schema:
                schema_object = json.load(schema)
                if generate_graph_wCustomSchema(Search_Context,schema_object)['Status Code'] == 200:
                    update_index(Search_Context, package_count)
        else:
